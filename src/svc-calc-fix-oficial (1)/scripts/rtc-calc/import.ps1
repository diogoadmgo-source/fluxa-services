...
try {
  ...
  # VersaoOutput oficial: { versaoApp, versaoDb, descricaoVersaoDb, dataVersaoDb, ambiente }
  $versao = $ver
  try { $j = $ver | ConvertFrom-Json; if ($j.versaoApp -and $j.versaoDb) { $versao = "$($j.versaoApp)-db$($j.versaoDb)" } elseif ($j.versaoApp) { $versao = $j.versaoApp } elseif ($j.versao) { $versao = $j.versao } } catch {}
